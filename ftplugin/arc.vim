...
" make comments behave as in c.vim
...
def update_lispwords():
    ...
    for line in vim.current.buffer:
        match = re.match('^\s*\(mac\s+(\S+)\s+(\(.+\.\s+body\)|body)', line)
        if match:
